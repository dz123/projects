...
def migrate_pirate_token_to_garth(
    ...
    print(f"Wrote long-lived OAuth2 token to {out}")
...
def main() -> int:
    ...
    if args.ticket:
        ...
        import platform
        # If on Mac without an interactive terminal (e.g. running via Shortcut in background)
        if platform.system() == "Darwin" and not sys.stdin.isatty():
            applescript = """
            tell application "System Events"
                activate
                set response to display dialog "Paste the full redirect URL (containing ticket=) or the ST-xxx ticket:" default answer "" with title "Garmin Login Authorization"
            ...
            try:
                ...
                raw = ""  # User clicked Cancel
        else:
            # Normal terminal — print instructions and prompt
            print(
                "After logging in via browser, copy the full redirect URL (containing ticket=...) from the address bar.",
                "You can also paste just the ST-...-sso ticket. It expires in ~1 minute, so act quickly.",
                sep="\n",
                flush=True,
            )
