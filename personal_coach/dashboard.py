...
with tab_train:
    ...
    # Calculate the index of the current week
    ...
    for i, w in enumerate(weeks):
        if w['start'] <= today_iso <= w['end']:
            ...
            default_idx = i  # If past the training plan, default to last week

    # Pass default_idx to selectbox
        ...
        for run in runs_only:
            ...
            with st.container(border=True):
                ...
                if f"report_{run_id}" in st.session_state:
                    ...
                    # 2. Render input box and handle new questions
                    if run_prompt := st.chat_input("Ask a follow-up about this run...", key=f"chat_input_{run_id}"):
                        # Save to local storage and display the user's question immediately
                        ...
                        with st.chat_message("assistant"):
                            with st.spinner("Coach is reviewing the telemetry..."):
                                # Send to this run_id's dedicated thread
                    ...
                    if st.button("Close Report", key=f"close_{run_id}"):
                        with st.spinner("Consolidating memory records..."):
                            # 1. Summarize the key takeaways from this conversation
                            chat_summary = agent.summarize_thread(f"run_analysis_{run_id}")
                            if chat_summary:
                                # 2. Append the summary to episodic memory
                ...
                if st.session_state.get('editing_run_id') == run_id:
                    ...
                    if laps:
                        if state_key not in st.session_state:
                            # Try to load previously saved lap categories
                        ...
                        with c1:
                            if st.button("Save & Calculate", type="primary", key=f"save_{run_id}"):
                                ...
                                # Pass the per-lap category list to the save function
